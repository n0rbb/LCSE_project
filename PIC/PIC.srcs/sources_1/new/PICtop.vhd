...
begin  -- behavior
  ...
  RAM_PHY: RAM_top
    port map (
        ...
        oe       => oe_mem, --OE activo alto, cuidado
        address  => address_mem,
        databus  => databus,
        Switches => switches,
        Temp_L   => Temp_L,
...
------ (2) ADDITIONAL MULTIPLEXERS TO READ/WRITE DIRECTLY IN RAM
   i_address_req <= i_write_en or (i_oe); --not (i_oe)
   address_mem <= i_address when (i_address_req = '1') else address;
   write_en_mem <= i_write_en or write_en;
   oe_mem <=  (i_oe or oe) and (not write_en_mem) ;   -- oe is active low (expression has been simplified)
