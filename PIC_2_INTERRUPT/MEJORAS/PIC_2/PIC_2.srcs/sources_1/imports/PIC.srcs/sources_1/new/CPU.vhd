...
process(current_state, FlagZ, Index_Reg, DMA_RQ, DMA_READY, ROM_Data, PC_reg, INS_reg, TMP_reg, INT, INT_ACK_flag, PC_reg_r, INS_reg_r, TMP_reg_r)
begin
		-- Valores por defecto
		Databus <= "ZZZZZZZZ";
		...
		case current_state is
            ...
            when Interrupt =>
                if INT_ACK_flag = '1' then
                    ...
                    --Salto a la dirección de memoria con lo que quiero que haga la interrupción de la interrupción
	                PC_reg_tmp <= "00001011"; --X"0B"; --Salto a W11
                    next_state <= Idle;
                else
                    -- Aquí podría ordenar a la ALU que guardase el contexto
                    INT_ACK <= '0';
...
BEGIN
	if reset='0' then
	    ...
	    if current_state = Interrupt and INT_ACK_flag = '0' then
	       ...
	       PC_reg_r <= PC_reg_tmp;
	       INS_reg_r <= INS_reg_tmp;
	       TMP_reg_r <= TMP_reg_tmp;
	       INT_ACK_flag <= '1';
	    else
	       INT_ACK_flag <= '0';	         
	    end if;
